...
Install (no admin rights needed):
  ...
  3. Start (or restart) Microsoft Word — a "Mirare Cite" tab appears
     on the ribbon.

Requirements:
  - Windows 10 (1809+) or Windows 11  (the required .NET Framework 4.8
    is already part of Windows — nothing else to download)
...
If the tab does not appear:
  - Fully quit Word (all windows) and start it again — Word only reads
    add-ins at startup.
  - Check the log:  %LOCALAPPDATA%\Mirare\addin.log
